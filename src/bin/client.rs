...
fn make_hello_request() -> DynRequest {
...
fn get_hello_response(buffer: &[u8]) -> DynResponse {
    let mut file_descriptor_protos = protobuf_parse::Parser::new()
        .protoc()
        .includes(&["proto"])
        .input("proto/helloworld.proto")
...
impl Decoder for DynCodec {
    ...
    fn decode(&mut self, src: &mut DecodeBuf<'_>) -> Result<Option<Self::Item>, Self::Error> {
        let resp = get_hello_response(src.chunk());
        Ok(Some(resp))
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = make_hello_request().into_request();
